...
import ListAnnonces from "../annonces.json"
...
const TextAnnonce = () => {
  ...
  const equipements = cetteAnnonce?.equipments.map((equipement,index)=>(
    <li className="listeEquipements" key={index}> {equipement} </li>
  ))

  const tags = cetteAnnonce?.tags.map((tag,index)=>(
    <div className="annonce_text_divEnTete_divTags_decoTag" key={index}>
      <p className="annonce_text_divEnTete_divTags_decoTag_text">{tag}</p>
    </div>
  ))
    ...
    <main className="annonce">
      ...
      <div className="annonce_text_divEnTete">
        <div className="annonce_text_divEnTete_divTitre">
          <h1 className="annonce_text_divEnTete_divTitre_titre">{cetteAnnonce?.title}</h1>
           <p className="annonce_text_divEnTete_divTitre_localisation">{cetteAnnonce?.location}</p>
        </div>
        <div className="annonce_text_divEnTete_divTags">{tags}</div>
      </div>
      <div className="annonce_text_divProprioRate">
        <div className="annonce_text_divProprioRate_proprietaire">
          <p className="annonce_text_divProprioRate_proprietaire_nom">{cetteAnnonce?.host.name}</p>
          <img  className="annonce_text_divProprioRate_proprietaire_photo" src={cetteAnnonce?.host.picture} alt="photo propriétaire"></img>
        </div>
